...
let PostProdutosService = class PostProdutosService {
    constructor(router, http) {
        ...
        this.http.get("/api/").subscribe((posts) => {
            for (let p of posts) {
                this.postsProduto.push(new _postProduto__WEBPACK_IMPORTED_MODULE_3__["PostProduto"](p.nome, p.preco, p.detalhes, p.modelo, p.mateterial, p.lavagem, p.arquivo, p.id));
            }
        });
    }
